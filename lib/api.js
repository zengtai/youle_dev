...
export function formatData(data) {
  ...
  if (EXCLUDED_GAMES && EXCLUDED_GAMES.length) {
    sourceData = sourceData.filter(
      (game) => !EXCLUDED_GAMES.includes(game.name)
    );
  }
  ...
  sourceData.map((game) => {
    let basicItem = {
      ...
      category: {
        ...
        slug: toFormatCategoryName(game.category)
          .toLowerCase()
          .replace(/ /g, `-`),
      },
      thumbnailUrl: `${IMAGE_PATH}webp/${game.name}.webp`,
    };
    ...
    fullItem.played = FEATURED_GAMES.includes(game)
      ? getCount("featured")
      : getCount();
...
export const getThumbnaiUrlBySlug = (slug) => {
  return `${IMAGE_PATH}webp/${slug}.webp`;
};
