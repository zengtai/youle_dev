...
export function formatData(data) {
  ...
  // if (SELECTED_GAMES && SELECTED_GAMES != null) {
  //   sourceData = sourceData.filter((game) =>
  //     SELECTED_GAMES.includes(game.name)
  //   );
  // }

  // if (EXCLUDED_GAMES && EXCLUDED_GAMES != null) {
  //   sourceData = sourceData.filter((game) =>
  ...
  sourceData.map((game) => {
    let basicItem = {
      ...
      category: {
        name: toFormat(game.category),
        slug: toFormat(game.category).toLowerCase(),
      },
  ...
  categories.map((category) => {
    let categoryData = basicData.filter(
...
export async function getRemoteContentLength() {
  return await fetch(`${process.env.NEXT_PUBLIC_API_URL}`).then(
    (res) => res.headers.get(`content-length`)
